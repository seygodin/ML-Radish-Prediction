# verify_mamba_detection_singlebox.md

- task: detection single-box, backbone: mamba
- status (reported): ok
...
- disease objectness: median=0.9990, p25=0.9974, min=0.0555
- normal  objectness: median=0.0000, p75=0.0000, p95=0.0037, max=0.9553
- normals scored >=0.5: **6/1303** (0.5%)
...
|---|---|---|---|
| det_pr_auc (primary) | 0.9975 | 0.9975 | ok |
| det_roc_auc | 0.9998 | 0.9998 | ok |
| presence_recall@0.5 | 0.9900 | 0.9900 | ok |
| fp_rate@0.5 | 0.0046 | 0.0046 | ok |
| iou_at_0.5_presence | 0.6300 | 0.6300 | ok |
| iou median (positives) | 0.6412 | 0.6412 | ok |
| iou mean (positives) | 0.5825 | 0.5825 | ok |
| map_at_0.5 | 0.5198 | 0.5242 | ok |

## 3) Image-level disease detection (primary interpretation)

- det_pr_auc = **0.9975**, det_roc_auc = **0.9998** (n_pos=100, n_neg=1303)
- presence_recall@0.5 (disease recall) = **0.990** = 99/100 (95% CI 0.946-0.998)
- fp_rate@0.5 (normal false alarm) = **0.0046** = 6/1303 (95% CI 0.0021-0.0100)

## 4) Localization quality (positives only, coarse-box caveat)

- IoU(pred,GT) on positive images (n=100): median=0.641, mean=0.583, p25=0.435, p75=0.728, min=0.099, max=0.992
- frac positives localized @IoU>=0.5 = 0.630
- mAP@0.5 (objectness-ranked, normal preds = FP) = **0.524** — interpret SEPARATELY from det_pr_auc: mAP couples ranking with coarse-box localization, so it is bounded by the IoU>=0.5 hit rate, not by disease/normal separability.
