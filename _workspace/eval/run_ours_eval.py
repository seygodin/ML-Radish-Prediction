#!/usr/bin/env python
"""eval-reporter (QA): Ours = DINOv3 ViT-S/16 (frozen) + 2-layer head.

Fair comparison vs from-scratch baseline best, on BOTH the original-distribution
valid set and the balanced valid set (1:1 / 1:1:1, seed=42). Independent metric
recomputation (sklearn cross-check), boundary cross-validation (predictions vs
manifest), params/trainable counts, +20% goal adjudication, verify files, JSON
dump, and a comparison figure.

best.pt weights are loaded for forward only (never modified). img_size=256
(from config.snapshot) is honored for the balanced re-evaluation.

Run: ./.venv/bin/python _workspace/eval/run_ours_eval.py
"""
...
# ---------------------------------------------------------------------------
# metric helpers (sklearn = independent cross-check vs run_eval primitives)
# ---------------------------------------------------------------------------
...
def baseline_best(setting):
    """Return dicts of best PR-AUC / best F1-macro baseline for orig + balanced."""
    out = {}
    # original-dist (recomputed in run_eval.py: prauc, f1_macro_recomp)
...
for setting in SETTINGS:
    ...
    assert m_model["arch"] == "dinov3"

    best_ep = int(mj["final"]["epoch"])
    # per_epoch is 0-indexed list; find the record with matching epoch
    ...
    # reported-vs-recomputed deltas (original dist)
    rep = mj["final"]
    rep_recomp = {
    ...
    # ---- (2) BALANCED: load best.pt, forward ----
    _, valid_loader, meta = build_classification_loaders(
        setting=setting, img_size=img_size, batch_size=64,
        num_workers=8, seed=SEED, balance_valid=True)
    model = build_classifier(arch="dinov3", num_classes=num_classes,
    ...
    # sklearn cross-check for normal_vs_d3 (orig): independent vs run_eval primitives
    ...
    results[name] = {
        "arch": "dinov3", "setting": setting, "num_classes": num_classes,
        "img_size": img_size, "best_epoch": best_ep,
    ...
    print(f"{name}: ORIG pr={orig['pr_auc']:.4f} f1={orig['f1_macro']:.4f} "
          ...
          f"BAL pr={bal['pr_auc']:.4f} f1={bal['f1_macro']:.4f} N={results[name]['bal']['n_valid']} | "
          f"params total={results[name]['params_total_M']}M train={results[name]['params_trainable_k']}k | "
          f"dist_match={dist_match}", flush=True)

# ---------------------------------------------------------------------------
# uplift vs baseline best, +20% goal adjudication
# ---------------------------------------------------------------------------
# design-notes 20% targets (3-class, orig dist): PR-AUC>=0.684, F1-macro>=0.851
GOAL = {"pr_auc": 0.684, "f1_macro": 0.851}

uplift = {}
for setting in SETTINGS:
    r = results[f"dinov3_{setting}"]
    bb = r["baseline_best"]
    u = {}
...
# 3-class goal adjudication (orig dist, design-notes targets)
r3 = results["dinov3_normal_d3_d4"]
goal_verdict = {
    "orig": {
...
dump = {"seed": SEED, "goal_targets_3class_orig": GOAL,
        "results": results, "uplift": uplift,
        "goal_verdict_3class": goal_verdict,
        "sklearn_crosscheck": sk_crosscheck}
(EVAL / "ours_dinov3.json").write_text(json.dumps(dump, indent=2, default=float))
print("\nwrote _workspace/eval/ours_dinov3.json")

# ---------------------------------------------------------------------------
# verify files (one per setting)
# ---------------------------------------------------------------------------
for setting in SETTINGS:
    r = results[f"dinov3_{setting}"]
    b = r["boundary"]; k = r["num_classes"]
    L = [f"# verify_dinov3_{setting}.md", "",
         f"- task: classification ({setting}), backbone: **dinov3 (DINOv3 ViT-S/16 frozen + 2-layer head)**, Ours",
         f"- img_size = **{r['img_size']}** (from config.snapshot), num_classes = {k}, best epoch = {r['best_epoch']}",
         f"- params: total = **{r['params_total_M']}M** (frozen backbone), trainable = **{r['params_trainable_k']}k** (head only)",
         ""]
    ...
    L += ["## 3) Balanced valid re-evaluation (best.pt loaded, weights unchanged; img_size=256)", "",
    ...
    # small-sample CI on disease recall (orig)
    ...
    (EVAL / f"verify_dinov3_{setting}.md").write_text("\n".join(L))
    print(f"wrote verify_dinov3_{setting}.md")

print("\nsklearn crosscheck (normal_vs_d3 orig):", json.dumps(sk_crosscheck, default=float))
print("3-class goal verdict (orig):", json.dumps(goal_verdict, default=float))
